...
print("Loading Nacho's LinkedIn data...")
input_file = "/Users/TERABYTE10/Downloads/Stacksync/04_linkedin_content/Stacksync Universe/Nacho/dataset_linkedin-post_2026-01-19_21-50-35-388.csv"
...
with open(input_file, 'r', encoding='utf-8-sig') as f:
    ...
    print(f"Columns: author={author_col}, likes={likes_col}, text={text_col}")
    ...
    for row_num, row in enumerate(reader):
        ...
        try:
            ...
            try:
                ...
                likes_num = comments_num = shares_num = followers_num = 0
...
sorted_authors = sorted(author_posts.items(), key=lambda x: -len(x[1]))[:25]
for author, likes_list in sorted_authors:
...
# Calculate for all authors
for author, likes_list in author_posts.items():
    if author not in author_avg:
        author_avg[author] = sum(likes_list) / len(likes_list) if likes_list else 1
...
# Save to CSV
output_file = "/Users/TERABYTE10/Downloads/Stacksync/04_linkedin_content/Stacksync Universe/Nacho/outliers_raw.csv"
with open(output_file, 'w', newline='', encoding='utf-8') as f:
    ...
    for i, post in enumerate(posts_sorted[:300]):
        writer.writerow([
            i+1, post['author'], post['followers'], post['likes'], post['comments'],
            post['shares'], post['total_engagement'], round(post['author_avg'], 1),
            round(post['outlier_score'], 2), post['hook'], post['text'], post['url'], post['date']
        ])
