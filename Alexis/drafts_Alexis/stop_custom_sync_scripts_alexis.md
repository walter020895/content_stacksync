...
→ A staging environment that's permanently out of sync with production because nobody wants to touch the sync config
...
You can write the script. You probably could write a better one than most off-the-shelf solutions. That's not the question.

The question is whether writing it is the best use of your team. In my experience, it almost never is.
...
- **Narrative arc**: Contrarian hook (stop writing) → qualifier (not because you can't) → six-item cost list (recognition) → "this is what it looks like" reframe → defense of the engineers (not a talent problem) → the real trade (maintenance vs. product) → close ("almost never worth it")
